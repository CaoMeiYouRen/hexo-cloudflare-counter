...
6. 已完成基础版写接口限流与短时去重，用于降低高频重复提交带来的影响。
7. 已完成来源白名单式 CORS 配置；跨域部署时可通过环境变量显式配置允许来源。
8. 已完成 SQLite 仓储，并接入 Node.js、Docker、Bun、当前 Vercel 路线。
9. 已完成 D1 仓储，并接入 Cloudflare Workers 路线。
10. 已完成 ObjectId 风格的 24 位十六进制 `objectId` 生成逻辑。
11. 已完成 requestId、日志、超时、Body 大小限制、CORS 与安全响应头等通用中间件装配。
12. 已完成基础测试、类型检查、lint 与构建链路。
13. 已完成 LeanCloud Counter JSONL -> SQLite 首版迁移脚本。
14. 已补充迁移脚本的基础测试与示例 JSONL 数据。
15. 已完成 LeanCloud Counter JSONL -> D1 的本地/远程迁移脚本，底层复用 Wrangler CLI 执行导入。
16. 已在文档中明确现阶段的兼容接入方式：现有 Hexo / NexT 项目主要只需把 `leancloud_visitors.server_url` 改为本服务地址；`server_url` 现为必填。
...
4. 增强限流、防刷量与安全增强能力，补齐多实例场景、可观测性与更细粒度策略。
5. 设计并实现部署期批量初始化 Counter 的管理能力，并把批量创建、状态查询、同步索引与数据修复能力一起纳入。
...
4. 对现有 Hexo / NexT 用户来说，当前迁移成本已经可以收敛到“部署后端并修改 `server_url`”这一主路径，但安全签名兼容和部署期管理能力仍待后续补齐。
